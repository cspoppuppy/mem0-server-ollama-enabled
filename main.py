...
# Embedder configuration from environment (supports ollama on host/docker)
OPEN_MODEL_PROVIDER = os.environ.get("MEM0_OPEN_MODEL_PROVIDER", "openai")
...
if OPEN_MODEL_PROVIDER.lower() == "ollama":
    llm_cfg = {"provider": "ollama", "config": {"model": LLM_MODEL, "temperature": 0, "max_tokens": 512, "ollama_base_url": OPEN_MODEL_BASE_URL}}
    embed_cfg = {"provider": "ollama", "config": {"model": EMBEDDER_MODEL, "ollama_base_url": OPEN_MODEL_BASE_URL}}
else:
    llm_cfg = {"provider": "openai", "config": {"api_key": OPENAI_API_KEY, "temperature": 0.2, "model": "gpt-4.1-nano-2025-04-14"}}
    embed_cfg = {"provider": "openai", "config": {"api_key": OPENAI_API_KEY, "model": EMBEDDER_MODEL}}
